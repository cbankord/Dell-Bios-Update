...
For v4.3 Windows Update, Dell Driver and PSADT section templates, use the
...
The latest v4 builder is **PSADT Deployment Builder v4.3**. Choose **BIOS update**
...
Check `BuildManifest.json`: `BuilderVersion` must be `4.3.0`; `PackageType` must
...
caption reads **PSADT Deployment Builder v4.3**. On **5 Build**, select **Output
...
Confirm `BuildManifest.json` records `BuilderVersion` `4.3.0` and the expected
...
3. Confirm `BuildManifest.json` has `BuilderVersion` equal to `4.3.0` (current), and generated
