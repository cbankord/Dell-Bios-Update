...
After staging, only explicit Restart Now requests a managed restart. Restart
Later/closing/timeout returns retry. There is no forced reboot countdown or future
OS shutdown timer. Intune must be set to No specific action. Monitor long-pending
staged updates because owned BitLocker suspension can remain until reboot; this
simpler design does not guarantee a restart deadline. Independent Windows/user
restarts cannot be power-gated by this script. The message tells the user to save
work, keep AC connected and wait for Windows to return.
...
Set the Intune installation timeout above three configured prompt timeouts plus
measured staging time and margin (120 minutes is a starting pilot value). The
third prompt occurs if a requested restart is held by a safety check. Never
shorten this budget by killing an updater.
...
- Preview/install/restart/info modes, logo/banner changes, keyboard/screen-reader
...
- Recovery-key backup/retrieval and finite BitLocker suspension immediately around
  staging; delayed restart, Restart Now safety checks, application-blocked restart,
